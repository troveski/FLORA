...
def augment_single_image_optimized(masked_image_path, original_label_path, output_image_dir, output_label_dir, 
                                 index, prompt, negative_prompt, guidance, steps, scheduler, models):
    """
    ...
    with torch.inference_mode():
        ...
        # Use the scheduler parameter instead of hardcoded "karras"
        sigmas = NODE_CLASS_MAPPINGS["BasicScheduler"]().get_sigmas(
            scheduler=scheduler, steps=steps, denoise=1, model=models['model']
        )
...
def main():
    ...
    parser.add_argument("--lora-strength-model", type=float, default=1.25, help="Strength of the LoRA applied to the UNET model.")
    ...
    parser.add_argument("--scheduler", type=str, default="karras", help="Scheduler type for sampling (e.g., 'karras', 'normal', 'simple', 'sgm_uniform', 'ddim_uniform', 'beta').")
